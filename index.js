...
window.onload = function () {
  ...
  document.addEventListener("mousedown", function (e) {
    ...
    lastAngle =
      (((Math.atan2(lastY - 512, lastX - 512) * 180) / Math.PI + 450) % 360) /
      90;
    lastDistance = Math.sqrt(
      Math.pow(lastY - 512, 2) + Math.pow(lastX - 512, 2)
    );
  });
  ...
  _('[name="parts"]').addEventListener("change", function (e) {
    ...
    if (e.target.value / 2 != Math.round(e.target.value / 2)) {
      return false;
    }
  ...
  function drawCanvas(e) {
    ...
    let thisAngle =
      (((Math.atan2(thisY - 512, thisX - 512) * 180) / Math.PI + 450) % 360) /
      90;
    let thisDistance = Math.sqrt(
      Math.pow(thisY - 512, 2) + Math.pow(thisX - 512, 2)
    );
  ...
  function download() {
    ...
    link.setAttribute(
      "href",
      draw.toDataURL("image/png").replace("image/png", "image/octet-stream")
    );
    link.click();
  }
};
